  ...
  <div>
    <div class="main">
      <div class="cdi" @click="cdiClick">
        <CDI :nav-current="navCurrent" :nav-flag="navFlag" :gs-current="gsCurrent" :gs-flag="gsFlag" />
        <div style="text-align: center; font-size: small">{{`Click here to ${showControls?'hide':'show'} the control panel.`}}</div>
      </div>
      <div class="controls" v-show="showControls">
        <label for="channel">Channel:</label>
        <select id="channel" v-model="selectedChannel">
          <option disabled value="">Please select one</option>
          <option v-for="f in channels" v-bind:key="f.name" v-bind:value="f.name">{{`${f.name} (${f.loc} / ${f.gp})`}}</option>
        </select><br/>
        ...
        <div class="meas">
          <div v-if="measurements['loc']" :set="m = measurements['loc']">
            LOC:<br/>
            <table>
            <tr><td>DDM:</td><td style="text-align:right;">{{(navCurrent).toFixed(1)}}&nbsp;</td><td>µA</td></tr>
            <tr><td>SDM:</td><td style="text-align:right;">{{m.sdm.toFixed(1)}}</td><td>%</td></tr>
            <tr><td>RF:</td><td style="text-align:right;">{{m.rf.toFixed(1)}}</td><td>dBFS</td></tr>
            </table>
          </div>
          <div v-else>
            LOC: No data
          ...
          <div v-if="measurements['gp']" :set="m = measurements['gp']">
            GP:<br/>
            <table>
            <tr><td>DDM:</td><td style="text-align:right;">{{(gsCurrent).toFixed(1)}}&nbsp;</td><td>µA</td></tr>
            <tr><td>SDM:</td><td style="text-align:right;">{{m.sdm.toFixed(1)}}</td><td>%</td></tr>
            <tr><td>RF:</td><td style="text-align:right;">{{m.rf.toFixed(1)}}</td><td>dBFS</td></tr>
            </table>
          </div>
  ...
  watch: {
    selectedChannel: function (val) {
      ...
      fetch("http://localhost:3344/channel", {
        ...
        .then(json => { console.log("New channel set, response: ", json) })
        .catch(e => { console.log(`error setting channel ${e}`) });
    },
  },
  computed: {
    channels: function() {
      // Return items to show in the channel selection dropdown list
      return allChannels.filter(i => (this.showYChannels || i.name.endsWith("X")) && i.loc != undefined);
    },
    navCurrent: function() {
      return this.measurements["loc"] ? this.measurements["loc"].ddm * 150/15.5 : 0;
    },
    navFlag: function() {
      let sdm_alarm = this.measurements["loc"]?.sdm < 30 || this.measurements["loc"]?.sdm > 50;
      return this.measurements["loc"] == undefined || sdm_alarm;
    },
    gsCurrent: function() {
      return this.measurements["gp"] ? this.measurements["gp"].ddm * 150/17.5 : 0;
    },
    gsFlag: function() {
      let sdm_alarm = this.measurements["gp"]?.sdm < 70 || this.measurements["gp"]?.sdm > 90;
